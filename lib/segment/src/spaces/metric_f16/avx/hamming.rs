...
pub unsafe fn avx_hamming_similarity_half(
    ...
    unsafe {
        ...
        while i < m {
            ...
            let v1_ps: __m256 = _mm256_cvtph_ps(addr1s);
            let v2_ps: __m256 = _mm256_cvtph_ps(addr2s);
            let cmp: __m256 = _mm256_cmp_ps(v1_ps, v2_ps, _CMP_NEQ_OQ);
            sum256_1 = _mm256_sub_ps(sum256_1, cmp);
            ...
            let v2_ps: __m256 = _mm256_cvtph_ps(addr2s);
            let cmp: __m256 = _mm256_cmp_ps(v1_ps, v2_ps, _CMP_NEQ_OQ);
            sum256_2 = _mm256_sub_ps(sum256_2, cmp);
            ...
            let v2_ps: __m256 = _mm256_cvtph_ps(addr2s);
            let cmp: __m256 = _mm256_cmp_ps(v1_ps, v2_ps, _CMP_NEQ_OQ);
            sum256_3 = _mm256_sub_ps(sum256_3, cmp);
            ...
            let v2_ps: __m256 = _mm256_cvtph_ps(addr2s);
            let cmp: __m256 = _mm256_cmp_ps(v1_ps, v2_ps, _CMP_NEQ_OQ);
            sum256_4 = _mm256_sub_ps(sum256_4, cmp);
        ...
        for i in 0..n - m {
            let val1 = f16::to_f32(*ptr1_f16.add(i));
            let val2 = f16::to_f32(*ptr2_f16.add(i));
            if (val1 - val2).abs() > f32::EPSILON {
                result += 1.0;
            }
        }
        -result
...
mod tests {
    ...
    fn test_spaces_avx() {
        use super::*;
        use crate::spaces::metric_f16::simple_hamming::*;
            ...
            && is_x86_feature_detected!("f16c")
        {
            ...
            let hamming_simd = unsafe { avx_hamming_similarity_half(&v1, &v2) };
            let hamming = hamming_similarity_half(&v1, &v2);
            assert!((hamming_simd - hamming).abs() / hamming.abs() < 0.0005);
        } else {
