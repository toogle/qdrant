...
pub unsafe fn neon_hamming_similarity_bytes(v1: &[u8], v2: &[u8]) -> f32 {
    ...
    unsafe {
        ...
        for _ in 0..len / 16 {
            ...
            let xor = veorq_u8(p1, p2);

            // Count set bits in each byte using population count
            let popcount_low = vcnt_u8(vget_low_u8(xor));
            let popcount_high = vcnt_u8(vget_high_u8(xor));

            // Widen to 16-bit and accumulate
            let popcount16_low = vmovl_u8(popcount_low);
            let popcount16_high = vmovl_u8(popcount_high);

            sum16_low = vaddq_u16(sum16_low, popcount16_low);
            sum16_high = vaddq_u16(sum16_high, popcount16_high);
        }
...
mod tests {
    ...
    fn test_spaces_neon() {
        if is_aarch64_feature_detected!("neon") {
            ...
            let hamming_simd = unsafe { neon_hamming_similarity_bytes(&v1, &v2) };
